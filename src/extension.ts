...
export function activate(context: vscode.ExtensionContext) {
  ...
  // Register the clipboardToFilePath command
  const clipboardToFilePathCommand = vscode.commands.registerCommand(
    "extension.clipboardToFilePath",
    async () => {
      await handleClipboardToFilePath();
    }
  );
  ...
  const uriHandler = vscode.window.registerUriHandler({
    handleUri(uri: vscode.Uri): vscode.ProviderResult<void> {
      ...
      // Check if the requested command is clipboardToFilePath
      if (uri.path === "/clipboardToFilePath") {
        // Execute the command
        vscode.commands.executeCommand("extension.clipboardToFilePath");
      }
    },
  });

  context.subscriptions.push(clipboardToFilePathCommand, uriHandler);
}

// Extract the functionality into a separate function
async function handleClipboardToFilePath() {
  try {
    ...
    vscode.window.showInformationMessage(`File written to: ${relativePath}`);
